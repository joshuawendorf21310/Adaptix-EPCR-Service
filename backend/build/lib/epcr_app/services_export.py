...
class NemsisExportService:
    ...
    async def generate_export(
        ...
        *,
        ...
        try:
            xml_bytes = f"<nemsis chart='{request.chart_id}' />".encode("utf-8")
            now = datetime.now(timezone.utc)
            ...
            attempt.artifact_storage_key = f"nemsis/{tenant_id}/{request.chart_id}/{attempt.id}.xml"
    ...
    async def retry_export(
        ...
        *,
        ...
        try:
            ...
            xml_bytes = f"<nemsis chart='{retry_attempt.chart_id}' />".encode("utf-8")
            ...
            retry_attempt.artifact_storage_key = (
                f"nemsis/{tenant_id}/{retry_attempt.chart_id}/{retry_attempt.id}.xml"
            )
