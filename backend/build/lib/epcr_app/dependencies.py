...
async def get_current_user(
    ...
    ``tid`` (tenant_id) claims. Signature verification is performed when
    ``ADAPTIX_JWT_PUBLIC_KEY`` is configured in the environment. In production
    ...
    if not public_key:
        logger.warning(
            "ADAPTIX_JWT_PUBLIC_KEY is not configured — JWT signature verification "
            "is disabled. This must be configured in production."
        )
