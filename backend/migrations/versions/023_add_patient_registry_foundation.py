...
def upgrade() -> None:
    ...
    if not _has_table(insp, "patient_registry_profiles"):
        op.create_table(
            ...
            sa.Column("ai_assisted", sa.Boolean(), nullable=False, server_default=sa.text("0")),
    ...
    if not _has_table(insp, "epcr_charting_accelerator_imports"):
        op.create_table(
            ...
            sa.Column("provider_confirmed", sa.Boolean(), nullable=False, server_default=sa.text("0")),
