"""Regression coverage for the live NEMSIS export pipeline."""
...
def test_state_dataset_builder_emits_state_dataset_and_mapped_elements(monkeypatch):
    ...
    monkeypatch.setenv("NEMSIS_SOFTWARE_NAME", "Receive and Process")
    monkeypatch.setenv("NEMSIS_SOFTWARE_VERSION", "2026.04.21")

    chart = SimpleNamespace(id="chart-1", call_number="CALL-100", created_at=datetime(2026, 4, 21, 12, 0, tzinfo=timezone.utc))
    mappings = [
    ...
    xml_bytes, warnings = NemsisXmlBuilder(chart=chart, mapping_records=mappings).build()
...
def test_validator_reports_missing_assets_as_deterministic_failure(monkeypatch):
    ...
    result = validator.validate_xml(b"<?xml version='1.0' encoding='UTF-8'?><StateDataSet xmlns='http://www.nemsis.org'/>")
...
def test_validator_resolves_state_dataset_xsd_from_official_zip(monkeypatch, tmp_path: Path):
...
def test_export_artifact_endpoint_returns_raw_xml_and_checksum():
    ...
    with patch(
        ...
        new=AsyncMock(return_value=(b"<?xml version='1.0'?><StateDataSet/>", "state-export.xml", "application/xml", "abc123")),
    ):
