...
class NemsisExportService:
    ...
    def _assert_transition(current: str, target: ExportLifecycleStatus):
        allowed = {
            "generation_requested": {"generation_in_progress"},
            "generation_in_progress": {"generation_succeeded", "generation_failed"},
        }
    ...
    async def generate_export(session, *, tenant_id, user_id, request: GenerateExportRequest):
        ...
        attempt = NemsisExportAttempt(
            ...
            status=ExportLifecycleStatus.GENERATION_REQUESTED.value,
            failure_type=None,
            message="Requested",
